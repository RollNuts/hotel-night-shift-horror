"""Quality gate for report-log filing pressure proof PNGs."""
...
MIN_FRAME_SPAN = 114
MIN_AVERAGE_LUMA_RANGE = 1.4
MIN_AVERAGE_RGB_ENERGY_RANGE = 3.6


def fail(message: str) -> None:
    raise RuntimeError(f"[ReportLogFilingPressureProofGate] {message}")


def frame_number_from_path(path: str) -> int | None:
    match = re.search(r"report_log_filing_pressure_proof_(\d+)\.png$", os.path.basename(path))
    if not match:
        return None
...
def main() -> None:
    ...
    parser.add_argument(
        "--capture-dir",
        default=os.path.join("Saved", "MovieRenders", "ReportLogFilingPressure"),
        help="Directory containing report-log filing pressure proof PNGs.",
    )
    args = parser.parse_args()

    paths = sorted(glob.glob(os.path.join(args.capture_dir, "report_log_filing_pressure_proof_*.png")))
    if not (MIN_EXPECTED_FRAME_COUNT <= len(paths) <= MAX_EXPECTED_FRAME_COUNT):
        fail(
            f"Expected {MIN_EXPECTED_FRAME_COUNT}-{MAX_EXPECTED_FRAME_COUNT} report-log proof PNGs "
    ...
    for path in paths:
        ...
        print(
            f"[ReportLogFilingPressureProofGate] {path}: average luma {metrics['average_luma']:.1f}, "
    ...
    if unique_hash_count < MIN_UNIQUE_HASHES:
        fail(
            f"Report-log proof PNGs are too static: {unique_hash_count}/{len(paths)} unique hashes, "
    ...
    if luma_range < MIN_AVERAGE_LUMA_RANGE or rgb_energy_range < MIN_AVERAGE_RGB_ENERGY_RANGE:
        fail(
            "Report-log proof PNGs have too little exposure/color variation; "
    ...
    print(
        f"[ReportLogFilingPressureProofGate] Passed {len(paths)} report-log filing pressure proof PNGs "
