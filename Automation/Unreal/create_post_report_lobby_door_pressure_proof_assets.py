"""Create a short post-report lobby-door pressure proof sequence.

This is an internal capture aid, not a gameplay feature or public trailer
approval. It frames the production front desk and lobby door so PRs can prove
the existing post-report "do not answer" beat reads as a physical hotel door
under pressure instead of a few blockout bars.
"""
...
SEQUENCE_NAME = "LS_PostReportLobbyDoorPressureProof_6s"
MRQ_CONFIG_NAME = "MRQ_PostReportLobbyDoorPressureProofPng"
...
CAMERA_KEYS = [
    (0, (560, -700, 164), (-2.0, 45.0, 0.0), 58.0),
    (18, (590, -670, 164), (-2.1, 44.2, -0.1), 56.0),
    (34, (620, -640, 163), (-2.2, 43.4, 0.1), 54.0),
    (56, (655, -610, 162), (-2.3, 42.6, -0.1), 52.0),
    (92, (690, -580, 161), (-2.4, 41.8, 0.0), 50.0),
    (END_FRAME, (730, -545, 160), (-2.5, 41.0, 0.0), 48.0),
]

DOOR_RATTLE_JOLTS = {
    "PROP_Lobby_GlassDoor_PostReportCrackWeb_Authored": (36, (-0.8, -2.6, 0.9), (0.7, 0.0, -1.5), 1.0),
    "PROP_Lobby_GlassDoor_PostReportLatchPlate_Authored": (42, (-1.4, -3.4, 1.0), (1.4, 0.0, 1.8), 1.0),
}

CONTEXT_SCALE_ACTORS = {
    "PROP_Surveillance_Monitor_PostReportFeedFrame": 0.72,
    ...
    "PROP_Lobby_GlassDoor_PostReportHoldClosedTape": 0.01,
    "PROP_Lobby_GlassDoor_PostReportOutsidePalmSmear": 0.01,
    "PROP_Lobby_GlassDoor_PostReportNoGuestReflection": 0.01,
    "PROP_Lobby_GlassDoor_PostReportHandleRattleBar": 0.01,
    "PROP_Lobby_GlassDoor_PostReportLatchRattlePin": 0.01,
    "PROP_Lobby_GlassDoor_PostReportTapeLooseEnd": 0.01,
    "LIGHTMESH_LobbyDoor_PostReportRattleCue": 0.01,
    "PROP_Lobby_GlassDoor_PostReportSmudgedPane_Authored": 0.01,
    "PROP_Lobby_GlassDoor_PostReportPalmSmear_Authored": 0.18,
    "PROP_Lobby_GlassDoor_PostReportTapeCross_Authored": 0.32,
}


def log(message: str) -> None:
    unreal.log(f"[PostReportLobbyDoorPressureProofAssets] {message}")


def fail(message: str) -> None:
    raise RuntimeError(f"[PostReportLobbyDoorPressureProofAssets] {message}")
...
def sanitize_sequence_metadata(sequence: unreal.LevelSequence) -> None:
    ...
    try_set_property(
        metadata,
        "notes",
        "Generated by Automation/Unreal/create_post_report_lobby_door_pressure_proof_assets.py for internal lobby-door pressure proof.",
    )
...
def add_spawnable_proof_camera(sequence: unreal.LevelSequence):
    ...
    temp_camera.set_actor_label("TMP_PostReportLobbyDoorPressureProofCamera")
    try:
        ...
        try_set_property(cine_component, "current_aperture", 4.0)
    ...
    binding.set_display_name("CAM_PostReport_LobbyDoorPressureProof_6s")
...
def add_actor_jolt_track(sequence: unreal.LevelSequence, label: str, jolt) -> None:
    ...
    if actor is None:
        fail(f"Missing lobby-door proof actor: {label}")
    ...
    settle_location = add_tuple(location, multiply_tuple(offset, -0.24))
    second_location = add_tuple(location, multiply_tuple(offset, 0.40))
    peak_rotation = add_tuple(rotation, rotation_delta)
    settle_rotation = add_tuple(rotation, multiply_tuple(rotation_delta, -0.30))
    second_rotation = add_tuple(rotation, multiply_tuple(rotation_delta, 0.36))
    peak_scale = multiply_tuple(scale, scale_multiplier)

    keys = [
        (0, location, rotation, scale),
        (max(0, start_frame - 5), location, rotation, scale),
        (start_frame + 2, peak_location, peak_rotation, peak_scale),
        (start_frame + 8, settle_location, settle_rotation, scale),
        (start_frame + 16, second_location, second_rotation, multiply_tuple(scale, 1.0 + (scale_multiplier - 1.0) * 0.45)),
        (start_frame + 32, location, rotation, scale),
...
def create_sequence() -> unreal.LevelSequence:
    ...
    if not sequence:
        fail("Unable to create post-report lobby-door pressure LevelSequence.")
    ...
    for label, jolt in DOOR_RATTLE_JOLTS.items():
...
def create_mrq_config() -> unreal.MoviePipelinePrimaryConfig:
    ...
    if not config:
        fail("Unable to create post-report lobby-door pressure MRQ config.")

    output = config.find_or_add_setting_by_class(unreal.MoviePipelineOutputSetting)
    output.output_resolution = unreal.IntPoint(1280, 720)
    output.file_name_format = "post_report_lobby_door_pressure_proof_{frame_number}"
    output.output_directory.path = "{project_dir}/Saved/MovieRenders/PostReportLobbyDoorPressure"
